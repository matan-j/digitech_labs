...
export default function BrandCoverUploader({ initialUrl }: { initialUrl: string | null }) {
  ...
  async function upload(file: File) {
    ...
    try {
      ...
      const res = await fetch('/api/admin/brand/cover', { method: 'POST', body: form });
      const data = await res.json();
      if (!res.ok) throw new Error(data.message ?? data.error ?? 'upload failed');
      setCoverUrl(data.url + '?v=' + Date.now());
      setState('success');
      router.refresh();
  ...
  async function remove() {
    ...
    setState('deleting');
    try {
      const res = await fetch('/api/admin/brand/cover', { method: 'DELETE' });
      const data = await res.json();
        ...
        {coverUrl ? (
          <div className="flex flex-col items-center gap-4">
            ...
            <img
              src={coverUrl}
              alt="Brand cover"
              className="w-full max-w-md rounded-card object-cover aspect-[16/7] border border-brand-purple-200"
            />
            <p className="text-xs text-neutral-500">תמונת הקאבר הנוכחית</p>
          </div>
        ) : (
          <div className="flex flex-col items-center gap-2 text-neutral-500">
            ...
            <p className="text-xs">PNG / JPG / WebP · עד 5MB · רצוי 1920×800+</p>
          </div>
        )}
